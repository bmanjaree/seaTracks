...
//working version with reshaping of the screen
...
//<script src="d3v7.min.js">
//<script src="http://d3js.org/d3.v7.min.js">
  async function loadFiles() {
    let tracks_PS_restructured = await d3.json("./tracks_full_restructured_14_11_12.json");
    let tracks_PS = await d3.json("./tracks_full_restructured_14_11_12.json");
    let coast_xy = await d3.json("./coast_xy.json");
    //let tracks_full_restructured = await d3.json("./tracks_full_restructured.json");
    //let tracks_full = await d3.json("./tracks_full_restructured.json");-->
    return [tracks_PS_restructured, tracks_PS, coast_xy
    //, tracks_full_restructured, tracks_full
    ];
  }
...
//Hardcoded values for Puget Sound
//var xmin = -125;
//var xmax = -121.75;
//var ymin = 46.75;
//var ymax = 50;

//Hardcoded values for full domain
var xmin = -135;
var xmax = -120;
var ymin = 39;
var ymax = 55;
...
//code adapted from https://brendansudol.com/writing/responsive-d3
function responsivefy(svg) {
    ...
    function resize() {
        ...
        //resize based on width and height depending on the aspect ratio
        if (targetWidth>width){
          var targetHeight = parseInt(container.style("height"));
          svg.attr("width", targetHeight/aspect);
          svg.attr("height", Math.round(targetHeight));
        }
...
// the one above is what we used before^ 
 var svg = d3.select("#vis")
    .append("svg")
    //.attr("viewbox", '0 0 700 1100')
    .attr("width", 700)
    .attr("height", 1200)
    .call(responsivefy);
...
scatterPlot.selectAll("circle.dot")
    ...
    .attr("fill", d => {
      if(selectedDrifter === d.track) {
        return "#3484c7"; // (NOT SURE THIS CONDITION IS WORKING)
      } else if (brushedDrifters.includes(d.track)) {
        return "purple";
      } else {
        return "blue";
    ...
    .attr("r", d=>{if (brushedDrifters.includes(d.track)){
      ...
      // else code plots full trajectory on selection, first clearing any previous selection:
        ...
        scatterPlot.selectAll("path.selected_float").remove();
      scatterPlot.append("path").attr("d", line(reorder_selected(selectedDrifter))).attr("fill", "none").attr("stroke", "green").attr("class","selected_float").attr("opacity",1);
      scatterPlot.selectAll("circle.dot")
      .data(scatterData, d => d.track)
      .attr("opacity", d => {
       if (brushedDrifters.includes(d.track)) {
